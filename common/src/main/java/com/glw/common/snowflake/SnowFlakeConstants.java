package com.glw.common.snowflake;
 ...
 * @Description : 常量接口
 */
public interface SnowFlakeConstants {
    /**
     * 启用的当前时间戳
     */
     ...
     * ※注：这四部分的位数可根据自身情况变动
     */
    long DATA_CENTER_BIT = 5L;       // 数据中心的占用位数（长度）
    long MACHINE_BIT = 5L;           // 机器标识的占用位数（长度）
    long SEQUENCE_BIT = 12L;         // 序列号的占用位数（长度）
